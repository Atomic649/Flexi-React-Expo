...
export const pdfExtract = async (req: Request, res: Response): Promise<void> => {
    // hardcoded file path
    const filePath = "/Volumes/LACIES/Flexi-React-Expo/python/pdf/AcctSt3.pdf";
    ...
    const formattedMatches = matches?.map(match => {
        ...
        const formattedDateTime = `20${year}-${month}-${day} ${time}:00.000`;
    ...
    const codeAmount = formattedMatches?.map(match => {
        ...
        const descMatch = descNoteMatches.find(descMatch => {
            ...
            const formattedDescDateTime = `20${descDate.split('/')[2]}-${descDate.split('/')[1]}-${descDate.split('/')[0]} ${descTime}:00.000`;
            return formattedDescDateTime === match.dateTime;
        });
        return { dateTime: match.dateTime, code: code.trim(), amount: parseInt(amount), desc: descMatch?.desc, note: descMatch?.note };
    });
    ...
    const businessAcc = await prisma.businessAcc.findFirst({
        ...
        if (!codeAmount) {
            res.status(400).json({ message: "No codeAmount data found" });
            return;
        }
